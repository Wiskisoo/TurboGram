...
public class UpdatesManager implements NotificationCenter.NotificationCenterDelegate {
    ...
    public void addCallback(UpdatesManagerCallback callback) {
        if (!callbacks.contains(callback)) {
            ...
            OctoLogging.d(TAG, "Adding callback: " + callback.getClass().getSimpleName());
        }

        if (callback.onGetStateAfterAdd()) {
            updateStateChange(callback);
        }
    }

    public void removeCallback(UpdatesManagerCallback callback) {
        if (callbacks.contains(callback)) {
            OctoLogging.d(TAG, "Removing callback: " + callback.getClass().getSimpleName());
    ...
    private InternalUpdateState getUpdateRecord() {
        ...
        boolean hasExtensionUpdate = false;
        ...
        return new InternalUpdateState(isCheckingForUpdates, isUpdateAvailable, isDownloading, downloadProgress, isReady, hasExtensionUpdate);
    }

    private void updateStateChange() {
        updateStateChange(null);
    }

    private void updateStateChange(UpdatesManagerCallback specialCallback) {
        ...
        for (UpdatesManagerCallback callback : callbacks) {
            ...
            if (specialCallback != null && specialCallback != callback) {
                continue;
            }
    ...
    private void checkIsUpdateAvailableAfterDataEnsure() {
        ...
        try {
            ...
            OctoLogging.e(TAG, "Failed to get package while app should be installed...", e);
        } catch (Exception e) {
    ...
    private boolean isResponseCorrupted(JSONObject object) {
        try {
            ...
            if (object.has("extension_update")) {
                JSONObject extensionUpdate = object.getJSONObject("extension_update");

                if (extensionUpdate.has("channel_username")) {
                    if (extensionUpdate.getString("channel_username").length() < 5) {
                        return true;
                    }
                } else {
                    return true;
                }
                if (extensionUpdate.has("versionCode")) {
                    extensionUpdate.getInt("versionCode");
                } else {
                    return true;
                }
                if (extensionUpdate.has("files")) {
                    JSONObject filesObject = extensionUpdate.getJSONObject("files");
                ...
                if (extensionUpdate.has("localized_changelogs")) {
                    extensionUpdate.getJSONObject("localized_changelogs");
                } else {
                    return true;
                }
            } else {
    ...
    public int getFirstAccountId() {
        if (!didInitFirstAccount) {
            ...
            for (int i = 0; i < UserConfig.MAX_ACCOUNT_COUNT; i++) {
                if (UserConfig.getInstance(i).getClientUserId() != 0 && !ConnectionsManager.getInstance(i).isTestBackend()) {
    ...
    private ExtensionUpdateState getExtensionStateFromObject(JSONObject object) {
        try {
            return new ExtensionUpdateState(
                    object.getInt("versionCode"),
                    object.getString("channel_username"),
                    object.getJSONObject("files").getInt("universal"),
                    getUpdateText(object)
            );
        } catch (JSONException e) {
            // considering the checks on object made by isUpdateCorrupted,
            // these fields are available for sure so we can run runtime exception
    ...
    public void installUpdate(boolean forceAndroidInstaller) {
        if (currentUpdateData != null) {
            ...
            AndroidUtilities.runOnUIThread(() -> ApkInstaller.install(currentUpdateData.document, getFirstAccountId(), forceAndroidInstaller));
        }
    }
    ...
    public interface UpdatesManagerCallback {
        ...
        default void onUpdateAvailable(TLRPC.TL_help_appUpdate update) {

        }
        default void onUpdateDownloading(float percent) {

        }
        default void onUpdateReady() {

        }

        //extension
        default void onExtensionUpdateAvailable(ExtensionUpdateState state, boolean hasAlsoClassicUpdate) {
    ...
    public record ExtensionUpdateState(int versionCode, String channelUsername, int messageID, String changelog) {

    }

    private record InternalUpdateState(boolean isChecking, boolean isUpdateAvailable, boolean isDownloading, float downloadProgress, boolean isReady, boolean hasExtensionUpdate) {
    ...
    public static class CheckCellState {
        ...
        public static final int UPDATE_IS_DOWNLOADING = 2;
        public static final int UPDATE_IS_READY = 3;
    }
}
